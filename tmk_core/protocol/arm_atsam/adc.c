...
uint8_t adc_config[][ADC_SAMPLEN + 1] = {{ADC_CONFIG_5V}, {ADC_CONFIG_C1A5}, {ADC_CONFIG_C1B5}, {ADC_CONFIG_C2A5}, {ADC_CONFIG_C2B5}, {ADC_CONFIG_C1I}, {ADC_CONFIG_C2I}};
...
__attribute__((weak)) void ADC0_clock_init(void) {
...
__attribute__((weak)) void ADC1_clock_init(void) {
...
__attribute__((weak)) void ADC0_load_cal(void) {
...
__attribute__((weak)) void ADC1_load_cal(void) {
...
__attribute__((weak)) void ADC_configure(uint8_t adc_index) {
    ...
    while (adc->SYNCBUSY.bit.SWRST) {
        DBGC(DC_ADC_SWRST_SYNCING_1);
    }
    while (adc->CTRLA.bit.SWRST) {
        DBGC(DC_ADC_SWRST_SYNCING_2);
    }
    ...
    while (adc->SYNCBUSY.bit.ENABLE) {
        DBGC(DC_ADC_ENABLE_SYNCING_1);
    }
}

// Return ADC_NA if the ADC is not available
// Return ADC counts otherwise
__attribute__((weak)) uint16_t adc_get(uint8_t adc_id) {
    uint16_t result = 0;
    ...
    if (adc_config[adc_id][ADC_PORT] == (uint8_t)ADC_NA) {
        ...
        while (adc->SYNCBUSY.bit.INPUTCTRL) {
            DBGC(DC_ADC_GET_INPUTCTRL_SYNCING_1);
        }
        ...
        if (adc->REFCTRL.bit.REFSEL != adc_config[adc_id][ADC_REFSEL]) {
            ...
            while (adc->SYNCBUSY.bit.REFCTRL) {
                DBGC(DC_ADC_GET_REFCTRL_SYNCING_1);
            }
        }
        ...
        if (adc->AVGCTRL.bit.SAMPLENUM != adc_config[adc_id][ADC_SAMPLENUM]) {
            ...
            while (adc->SYNCBUSY.bit.AVGCTRL) {
                DBGC(DC_ADC_GET_AVGCTRL_SYNCING_1);
            }
            if (adc->AVGCTRL.bit.SAMPLENUM == ADC_AVGCTRL_SAMPLENUM_1_Val)
                adc->AVGCTRL.bit.ADJRES = 0;
            else if (adc->AVGCTRL.bit.SAMPLENUM == ADC_AVGCTRL_SAMPLENUM_2_Val)
                adc->AVGCTRL.bit.ADJRES = 1;
            else if (adc->AVGCTRL.bit.SAMPLENUM == ADC_AVGCTRL_SAMPLENUM_4_Val)
                adc->AVGCTRL.bit.ADJRES = 2;
            else if (adc->AVGCTRL.bit.SAMPLENUM == ADC_AVGCTRL_SAMPLENUM_8_Val)
                adc->AVGCTRL.bit.ADJRES = 3;
            else
                adc->AVGCTRL.bit.ADJRES = 4;
            while (adc->SYNCBUSY.bit.AVGCTRL) {
                DBGC(DC_ADC_GET_AVGCTRL_SYNCING_2);
            }
        }
        ...
        if (adc->SAMPCTRL.bit.SAMPLEN != adc_config[adc_id][ADC_SAMPLEN]) {
            ...
            while (adc->SYNCBUSY.bit.SAMPCTRL) {
                DBGC(DC_ADC_GET_SAMPCTRL_SYNCING_1);
            }
        }

        adc->SWTRIG.bit.START = 1;
        while (adc->SYNCBUSY.bit.SWTRIG) {
            DBGC(DC_ADC_GET_SWTRIG_SYNCING_1);
        }
        while (!adc->INTFLAG.bit.RESRDY) {
            DBGC(DC_ADC_GET_INTFLAG_RESRDY_WAITING_1);
        }
