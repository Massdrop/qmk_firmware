...
void SR_KC_WriteData(void) {
    ...
    while (!(SR_KC_SERCOM->SPI.INTFLAG.bit.DRE)) {
        DBGC(DC_SPI_KC_WRITE_DRE);
    }

    /* Shift in bits 8 - 15 */
    SR_KC_SERCOM->SPI.DATA.bit.DATA = (sr_kc_data.reg >> 8) & 0xFF;
    while (!(SR_KC_SERCOM->SPI.INTFLAG.bit.TXC)) {
        DBGC(DC_SPI_KC_WRITE_TXC_1);
    }

    /* Shift in bits 0 - 7 */
    SR_KC_SERCOM->SPI.DATA.bit.DATA = sr_kc_data.reg & 0xFF;
    while (!(SR_KC_SERCOM->SPI.INTFLAG.bit.TXC)) {
        DBGC(DC_SPI_KC_WRITE_TXC_2);
    }
...
void SR_KC_Init(void) {
    ...
    while (SR_KC_SERCOM->SPI.SYNCBUSY.bit.ENABLE) {
        DBGC(DC_SPI_KC_SYNC_ENABLING);
    }
